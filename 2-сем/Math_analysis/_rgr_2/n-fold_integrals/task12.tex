...
\begin{figure}[h]
	...
	\begin{tikzpicture}
	\begin{axis}
    		\addplot [domain=0:2*pi, samples=100, thick, name path=e2] ({exp(1)*cos(deg(x))}, {exp(1)*sin(deg(x))});
    		\addplot [domain=0:2*pi, samples=100, thick, name path=e4] ({exp(2)*cos(deg(x))}, {exp(2)*sin(deg(x))});
	\addplot [
		thick,
